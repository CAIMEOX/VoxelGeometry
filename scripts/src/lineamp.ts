...
class Matrix {
  ...
  constructor(r: number, c: number, val: number = 0) {
    ...
    for (let i = 1; i <= r; ++i) {
      ...
      for (let j = 1; j <= c; ++j) this.matrix[i][j] = val;
    }
  }
  /**交换矩阵的a和b两行*/
  ...
  mul(r: number, k: number): void {
    ...
    for (let i = 1; i <= this.column; ++i) this.matrix[r][i] *= k;
  }
  /**将矩阵第a行所有元素乘以一个数字后加到第b行(第a行元素不发生改变)*/
  add(a: number, b: number, k: number): void {
    if (a > this.row || b > this.row) throw new Error("The row is too big");
    for (let i = 1; i <= this.column; ++i) this.matrix[b][i] += this.matrix[a][i] * k;
  }
  /**对矩阵进行水平翻转 */
  fliphorizontal(): void {
    for (let i = 1; i <= this.row / 2; ++i) this.swap_row(i, this.row - i + 1);
  }
  /**对矩阵进行垂直翻转 */
  flipvertica(): void {
    for (let i = 1; i <= this.column / 2; ++i) this.swap_column(i, this.column + 1);
  }
  /**对矩阵进行主对角线翻转 */
  flipmdiagonal(): void {
  ...
  inv(mod: number): Matrix {
    ...
    let l = new Matrix(this.row, this.row),
      u = new Matrix(this.row, this.row),
      ans = new Matrix(this.row, this.row);
    for (let i = 1; i <= this.row; ++i) {
      l.matrix[i][i] = 1;
    ...
    for (let i = 2; i <= this.row; ++i)
      for (let j = i; j <= this.row; ++j) {
        ...
        for (let k = 1; k <= i - 1; ++k)
          u.matrix[i][j] = (u.matrix[i][j] + mod - ksc(l.matrix[i][k], u.matrix[k][j])) % mod;
        l.matrix[j][i] = this.matrix[j][i];
        for (let k = 1; k <= i - 1; ++k)
          l.matrix[j][i] = (l.matrix[j][i] + mod - ksc(l.matrix[j][k], u.matrix[k][i])) % mod;
  ...
  toString(): string {
    let result = "";
    for (let i = 1; i <= this.row; ++i) {
      for (let j = 1; j <= this.column; ++j) result += this.matrix[i][j].toString() + " ";
      result += "\n";
    }
    return result;
  }
...
namespace operation {
  ...
  export function add(a: Matrix, b: Matrix): Matrix {
    ...
    let result = a;
    for (let i = 1; i <= a.row; ++i) for (let j = 1; j <= a.column; ++j) result.matrix[i][j] += b.matrix[i][j];
    return result;
  }
  /**计算两个矩阵的差 */
  ...
  export function mul(a: Matrix, b: Matrix): Matrix {
    ...
    for (let i = 1; i <= a.row; ++i)
      for (let k = 1; k <= a.column; ++k)
        for (let j = 1; j <= b.column; ++j) result.matrix[i][j] += a.matrix[i][k] * b.matrix[k][j];
    return result;
  }
  /**采用快速幂方法计算方形矩阵的幂a^p */
  ...
  export function equal(a: Matrix, b: Matrix): boolean {
    ...
    for (let i = 1; i < a.row; ++i)
      for (let j = 1; j < a.column; ++j) if (a.matrix[i][j] != b.matrix[i][j]) return false;
    return true;
  }
}
