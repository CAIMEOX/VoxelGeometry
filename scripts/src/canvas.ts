...
function calcMinimumBounds(lines: any[]) {
  let bounds = { x: Number.MAX_VALUE, y: Number.MAX_VALUE, x2: Number.MIN_VALUE, y2: Number.MIN_VALUE };
...
function calcSortedIntersections(lines: string | any[], y: number) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if ((A.y < y && B.y >= y) || (B.y < y && A.y >= y)) {
      let xval: number = A.x + ((y - A.y) / (B.y - A.y)) * (B.x - A.x);
      xlist.push(xval);
    }
  }
...
function pathToLines(path: any[]) {
  ...
  path.forEach(function (cmd: any[]) {
    ...
    if (cmd[0] == PATH_COMMAND.QUADRATIC_CURVE) {
      let pts: Point[] = [curr, cmd[1], cmd[2]];
      for (let t = 0; t < 1; t += 0.1) {
        let pt: Point = calcQuadraticAtT(pts, t);
        lines.push(new Line(curr!, pt));
        curr = pt;
      }
    }
    if (cmd[0] == PATH_COMMAND.BEZIER_CURVE) {
      let pts: Point[] = [curr, cmd[1], cmd[2], cmd[3]];
      for (let t = 0; t < 1; t += 0.1) {
        let pt = calcBezierAtT(pts, t);
        lines.push(new Line(curr!, pt));
        curr = pt;
      }
    }
...
export { Transform, Context, Bitmap, Point, Line };
>>>>>>> 3cfc334d4db097b50d278e446f8cb1cf44de34b5
